...
@section('content')
    ...
    <div class="response__box">
        <div class="response__inner">
            <h1 class="response__title">По запросу "{{ $s }}"</h1>
            @if ($films->count() > 0)
                <p class="response__txt">Найдено {{ $films->count() }} фильмов.</p>
            @else
                <p class="response__txt">Ничего не найдено.</p>
            @endif
        </div>
    </div>
    ...
    <div class="content">
        ...
        <!-- Pagination -->
        @if ($films->appends(['s' => request()->s ])->hasPages())
            <ul class="pagination">
